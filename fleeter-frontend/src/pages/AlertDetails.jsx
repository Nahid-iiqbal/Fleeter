...
import { getAlertStatusColor, getAlertTypeColor, getAlertTypeLabel } from '../utils/alerts';
  ...
  useEffect(() => {
    const loadAlert = async () => {
      try {
        setLoading(true);
        setError('');
        const data = await apiFetch(`/api/company/alerts/${alertType}/${alertId}`);
        setAlert(data);
      } catch (loadError) {
      ...
      <Box>
        ...
        <Card elevation={0} sx={{ border: 1, borderColor: 'divider' }}>
          <CardContent sx={{ p: 4 }}>
            ...
            <Alert severity="error">{error || 'This alert could not be found.'}</Alert>
          </CardContent>
        </Card>
      </Box>
    );
  ...
  return (
    <Box sx={{ display: 'flex', flexDirection: 'column', gap: 3, pb: 4 }}>
      <Paper elevation={0} sx={{ borderBottom: 1, borderColor: 'divider', p: 3, mx: -3, mt: -3, mb: 1, borderRadius: 0, bgcolor: 'background.paper', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        <Box>
          <Typography variant="h5" fontWeight={700}>Alert Details</Typography>
          <Typography variant="body2" color="text.secondary">{getAlertTypeLabel(alert.alert_type)}</Typography>
        </Box>
      </Paper>
      ...
      <Card elevation={0} sx={{ border: 1, borderColor: 'divider' }}>
        <CardContent sx={{ p: 3, display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: 2 }}>
          <Box sx={{ display: 'flex', alignItems: 'center', gap: 2.5 }}>
            <Avatar sx={{ width: 60, height: 60, bgcolor: `${getAlertTypeColor(alert.alert_type)}.main`, color: '#fff', fontWeight: 700 }}>
              {getAlertTypeLabel(alert.alert_type).charAt(0).toUpperCase()}
            </Avatar>
            <Box>
              <Typography variant="h5" fontWeight={700}>{alert.title || getAlertTypeLabel(alert.alert_type)}</Typography>
              <Typography variant="body1" color="text.secondary">{alert.about}</Typography>
            </Box>
          </Box>
          <Box sx={{ display: 'flex', gap: 1, flexWrap: 'wrap' }}>
            <Chip label={getAlertTypeLabel(alert.alert_type)} color={getAlertTypeColor(alert.alert_type)} sx={{ fontWeight: 700 }} />
            <Chip label={alert.status} color={getAlertStatusColor(alert.status)} sx={{ fontWeight: 700, textTransform: 'capitalize' }} />
          </Box>
        </CardContent>
      </Card>
      ...
      <Card elevation={0} sx={{ border: 1, borderColor: 'divider' }}>
        <CardContent sx={{ p: 3 }}>
          <Typography variant="h6" fontWeight={700} mb={3}>Alert information</Typography>
      ...
      <Card elevation={0} sx={{ border: 1, borderColor: 'divider' }}>
        <CardContent sx={{ p: 3 }}>
          ...
          <Typography variant="body1" color="text.primary">
            {alert.description || 'No additional description was provided for this alert.'}
          </Typography>
        </CardContent>
      </Card>
    </Box>

  ...
  if (loading) {
    return (
      <Box sx={{ display: "flex", flexDirection: "column", alignItems: "center", py: 4 }}>
        ...
        <Typography color="text.secondary">Loading company requests...</Typography>
      </Box>
    );
  }
            ...
            <Grid container spacing={4}>
              ...
              <Grid item xs={12} md={5}>
                <Box sx={{ mb: 3, display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
                  <Box>
                    <Typography variant="h6" fontWeight={700}>Make a request</Typography>
                    <Typography variant="body2" color="text.secondary" mt={0.5}>
                      Send one request and wait for the company&apos;s response.
                    </Typography>
                  </Box>
                  <Chip
                    label={requestLocked ? "Request pending" : "Ready to request"}
                ...
                {requestLocked && (
                  <Alert severity="warning" sx={{ mb: 3 }}>
                    ...
                    You can request another company after cancelling the pending request.
                  </Alert>
                )}
                  ...
                  <Stack spacing={2}>
                    ...
                    >
                      ...
                      {companies.map((company) => (
                        <MenuItem key={company.owner_id} value={company.owner_id}>
              ...
              <Grid item xs={12} md={7}>
                <Box sx={{ p: 3, bgcolor: "background.default", borderRadius: 2, height: "100%", border: 1, borderColor: "divider" }}>
                  <Box sx={{ display: "flex", justifyContent: "space-between", alignItems: "center", mb: 3 }}>
                    <Box>
                      <Typography variant="h6" fontWeight={700} display="flex" alignItems="center" gap={1.5}>
                        Your requests
                        {mine.length > 0 && <Badge badgeContent={mine.length} color="primary" />}
                      </Typography>
      ...
      {!joinOnly && (role === "owner" || role === "manager") && (
        <Card elevation={0} sx={{ border: 1, borderColor: "divider" }}>
          <Box sx={{ p: 3, display: "flex", justifyContent: "space-between", alignItems: "center", borderBottom: 1, borderColor: "divider" }}>
            ...
            <Button onClick={loadRequests} disabled={loading} startIcon={loading ? <CircularProgress size={16} /> : <RefreshIcon />}>
              Refresh
            </Button>
          </Box>
          <CardContent sx={{ p: 0 }}>
            {pending.length === 0 ? (
              ...
              <Stack divider={<Divider />}>
                {pending.map((request) => (
                  <Box key={request.request_id} sx={{ p: 3, display: "grid", gridTemplateColumns: { xs: "1fr", md: "1.5fr 2fr auto" }, gap: 3, alignItems: "center", "&:hover": { bgcolor: "action.hover" } }}>
                    ...
                    <Box>
                      ...
                      {request.message && (
                        <Paper elevation={0} sx={{ p: 1.5, bgcolor: "background.default", border: 1, borderColor: "divider", borderRadius: 1 }}>
                          <Typography variant="caption" fontWeight={700} display="block" color="text.secondary" mb={0.5}>Note</Typography>
                    ...
                    <Box sx={{ display: "flex", gap: 1, justifyContent: { xs: "flex-start", md: "flex-end" } }}>
                      <Button variant="contained" color="success" size="small" onClick={() => decide(request.request_id, "approve")}>
                        Approve
                      </Button>
                      <Button variant="contained" color="error" size="small" onClick={() => decide(request.request_id, "reject")}>
                        Reject
                      </Button>
                    </Box>
...
function RequestList({ requests, empty, onCancel }) {
  if (requests.length === 0) {
    return <Typography color="text.secondary" sx={{ fontStyle: "italic" }}>{empty}</Typography>;
  }

  return (
    <Stack spacing={2}>
      {requests.map((request) => (
        <Paper key={request.request_id} elevation={0} sx={{ p: 2, border: 1, borderColor: "divider", display: "grid", gridTemplateColumns: { xs: "1fr", md: "1.2fr 1fr auto" }, gap: 2, alignItems: "center" }}>

          <Box sx={{ display: "flex", gap: 2, alignItems: "center" }}>
            <CustomAvatar name={request.company_name || "Company"} image={request.company_profile_image} />
            <Box>
              <Typography fontWeight={600}>{request.company_name || "Company"}</Typography>
              <Typography variant="body2" color="text.secondary">
                {request.owner_username ? `Owner: ${request.owner_username}` : "Company details unavailable"}
              </Typography>
              {request.owner_email && <Typography variant="caption" color="text.secondary">{request.owner_email}</Typography>}
            </Box>
          </Box>
          ...
          <Box>
            ...
            {request.message && (
              <Typography variant="body2" color="text.secondary" sx={{ bgcolor: "action.hover", p: 1, borderRadius: 1 }}>
                "{request.message}"
              </Typography>
            )}
          </Box>

          <Box sx={{ display: "flex", gap: 1.5, alignItems: "center", justifyContent: { xs: "flex-start", md: "flex-end" } }}>
            <Chip
            ...
            {request.status === "pending" && (
              <Button size="small" variant="outlined" color="inherit" onClick={() => onCancel(request.request_id)}>
                Cancel
              </Button>
            )}
          </Box>
...
function CustomAvatar({ name, image }) {
  ...
  const initials = (name || "?")
    ...
    .join("");
  return (
    <Avatar sx={{ width: 46, height: 46, bgcolor: "primary.light", color: "primary.dark", fontWeight: 700 }}>
      {initials}
    </Avatar>
  );
}
